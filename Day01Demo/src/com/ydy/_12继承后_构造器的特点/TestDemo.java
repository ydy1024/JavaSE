package com.ydy._12继承后_构造器的特点;

/**
 目标：继承后-构造器的特点

 特点：
    子类构造器的第一行默认一定先访问父类的无参数构造器，再执行子类自己的构造器。
 为什么子类有参构造器会先调用父类构造器？
    1.子类构造器的第一行默认有一个super()调用父类的无参数构造器，写不写都存在！
    2.子类继承父类，子类得到了父类的属性和行为。
