package com.ydy._02static关键字;

/**
 目标：static关键字的概述（重点）

 引入：
    我们之前定义了很多成员变量（name，age，sex）其实我们只写了一份，
    但是发现每个对象都可以使用，就说明Java中这些成员变量或者方法都存在所属性的。
    有些是属于对象的，有些是属于类本身的。
 ...
 小结：
    成员变量有2种：
        --有static修饰的属于类，属于静态成员变量，与类一起加载一次，直接用类名即可调用。
        --无static修饰的属于类的每一个对象，叫实例成员变量，
          与类的对象一起加载，对象有多少个，实例成员变量就加载多少份。必须用类的对象调用。
    成员方法有2种：
        --有static修饰的属于类，叫静态方法，直接用类名调用即可。
        --无static修饰的属于类的每个对象，叫实例方法，必须用类的对象调用。
